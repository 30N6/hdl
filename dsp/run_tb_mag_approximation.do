...
set tb_lib    dsp_lib
set tb_name   mag_approximation_tb
...
set library_file_list [list \
  ...
  dsp_lib [list \
    ./hdl/mag_approximation.vhd \
    ./sim/mag_approximation_tb.sv] \
]
  ...
  foreach file $file_list {
    if { $last_compile_time < [file mtime $file] } {
      if [regexp {.vhdl?$} $file] {
        ...
        #vlog +define+SIM -sv -timescale "1 ns / 1 ns" -L common_lib -L dsp_lib -work $library $file {*}[split $incdir_str " "]
      }
      set last_compile_time 0
    }
  }
...
#eval vsim $top_level
#eval vsim -L common_lib -L dsp_lib work.$tb_name
