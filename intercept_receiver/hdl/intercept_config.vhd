...
architecture rtl of intercept_config is
  ...
  signal w_module_id            : unsigned(INTERCEPT_MODULE_ID_WIDTH - 1 downto 0);
  signal w_message_type         : unsigned(INTERCEPT_MESSAGE_TYPE_WIDTH - 1 downto 0);
  signal w_address              : unsigned(INTERCEPT_CONFIG_ADDRESS_WIDTH - 1 downto 0);
  signal r_module_id            : unsigned(INTERCEPT_MODULE_ID_WIDTH - 1 downto 0);
  signal r_message_type         : unsigned(INTERCEPT_MESSAGE_TYPE_WIDTH - 1 downto 0);
  ...
  signal w_mod_config_data      : std_logic_vector(INTERCEPT_CONFIG_DATA_WIDTH - 1 downto 0);
...
begin
  ...
  w_module_id     <= unsigned(r_axis_data(24 + INTERCEPT_MODULE_ID_WIDTH - 1 downto 24));
  w_message_type  <= unsigned(r_axis_data(16 + INTERCEPT_MESSAGE_TYPE_WIDTH - 1 downto 16));
  w_address       <= unsigned(r_axis_data(INTERCEPT_CONFIG_ADDRESS_WIDTH - 1 downto 0));
    ...
    if rising_edge(S_axis_clk) then
      if (S_axis_resetn = '0') then
        ...
        if (r_axis_valid = '1') then
          case s_state is
          when S_WORD_0 =>
            if (r_axis_data = INTERCEPT_CONTROL_MAGIC_NUM) then
              s_state <= S_WORD_1;
            else
              s_state <= S_DROP;
            end if;
          ...
          when S_WORD_3 =>
            if (r_module_id = INTERCEPT_MODULE_ID_CONTROL) then
  ...
  generic map (
    FIFO_DEPTH  => 16,
    FIFO_WIDTH  => INTERCEPT_CONFIG_DATA_WIDTH
  )
  port map (
    Clk_wr        => S_axis_clk,
    Clk_rd        => Clk_x4,
  ...
  begin
    if rising_edge(S_axis_clk) then
      if (S_axis_resetn = '0') then
        ...
        if ((r_axis_valid = '1') and (s_state = S_ACTIVE_CONFIG_CONTROL) and (r_first = '1') and (r_message_type = INTERCEPT_CONTROL_MESSAGE_TYPE_ENABLE)) then
