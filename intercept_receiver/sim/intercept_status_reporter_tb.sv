...
typedef struct {
  ...
  intercept_dwell_stats_errors_t      dwell_stats_errors;
  intercept_status_reporter_errors_t  status_reporter_errors;
  int                                 pre_write_delay;
} intercept_status_data_t;

typedef intercept_status_data_t intercept_status_data_array_t [];
...
interface intercept_status_tx_intf (input logic Clk);
  ...
  intercept_stream_encoder_errors_t   stream_encoder_errors;
  intercept_dwell_stats_errors_t      dwell_stats_errors;
  //intercept_dwell_controller_errors_t dwell_controller_errors;  //TODO
  ...
  task write(intercept_status_data_t input_data);
...
module intercept_status_reporter_tb;
  ...
  typedef struct packed
  {
    ...
    bit [63:0]  timestamp;
  } intercept_status_report_header_t;

  typedef struct packed
  {
    bit error_status_reporter_overflow;
    bit error_status_reporter_timeout;
  } intercept_status_reporter_errors_packed_t;
  ...
  typedef bit [$bits(intercept_status_flags_packed_t) - 1 : 0]            intercept_status_flags_packed_bits_t;
  typedef bit [$bits(intercept_status_reporter_errors_packed_t) - 1 : 0]  intercept_status_reporter_errors_packed_bits_t;

  parameter NUM_HEADER_WORDS = ($bits(intercept_status_report_header_t) / AXI_DATA_WIDTH);

  logic Clk_axi;
  logic Clk;
  logic Rst;

  intercept_status_tx_intf                        status_tx_intf (.*);
  ...
  function automatic intercept_status_report_header_t unpack_report_header(logic [AXI_DATA_WIDTH - 1 : 0] data [$]);
    intercept_status_report_header_t      report_header;
    intercept_status_report_header_bits_t packed_report_header;
    ...
    report_header = intercept_status_report_header_t'(packed_report_header);
    return report_header;
  endfunction


  function automatic bit data_match(logic [AXI_DATA_WIDTH - 1 : 0] a [$], logic [AXI_DATA_WIDTH - 1 : 0] b []);
    intercept_status_report_header_t report_a = unpack_report_header(a);
    intercept_status_report_header_t report_b = unpack_report_header(b);
    ...
    for (int i = NUM_HEADER_WORDS; i < intercept_max_words_per_packet_small; i++) begin
      if (a[i] !== b[i]) begin
  ...
  function automatic void expect_report(intercept_status_data_t input_data);
    int                                             num_padding_words = 0;
    expect_t                                        r;
    intercept_status_report_header_t                report_header;
    intercept_status_report_header_bits_t           report_header_packed;
    intercept_status_reporter_errors_packed_t       reporter_errors;
    intercept_status_reporter_errors_packed_bits_t  reporter_errors_packed;
    intercept_status_flags_packed_t                 status_flags;
    intercept_status_flags_packed_bits_t            status_flags_packed;
    ...
    report_header.module_id         = intercept_module_id_status;
    report_header.message_type      = intercept_report_message_type_status;
    report_header.enables           = {27'h0, input_data.enable_stream_encoder, input_data.enable_channelizer, 1'b1};
    report_header.status_path_0     = status_flags_packed;
    report_header.status_reporter   = reporter_errors;
    report_header.timestamp         = 0;

    report_header_packed = intercept_status_report_header_bits_t'(report_header);
    ...
    num_padding_words = intercept_max_words_per_packet_small - r.data.size();
  ...
  function automatic intercept_status_data_array_t randomize_status_input(int num_packets);
    intercept_status_data_array_t r = new [num_packets];
  ...
  task automatic standard_test();
    ...
    for (int i_test = 0; i_test < NUM_TESTS; i_test++) begin
      intercept_status_data_array_t status_input = randomize_status_input(HEARTBEATS_PER_TEST);
